...
The notebook begins by creating a **`Session`** object from `sagemaker.core.helper.session_helper`. This is the entry point for all SageMaker SDK calls. It resolves your default S3 bucket, reads your IAM execution role, and holds the AWS region. Every other SDK object in this lab (`SFTTrainer`, `DataSet`, `CustomScorerEvaluator`) accepts a `sagemaker_session` parameter that ties it to this session.

:::alert{header="About `Session`" type="info"}
`sagemaker.core.helper.session_helper.Session` is the SageMaker Python SDK session object. It wraps a `boto3.Session` and provides defaults (bucket, region, role) so you don't repeat them on every API call. [SDK documentation](https://sagemaker.readthedocs.io/en/stable/api/utility/session.html)
:::
...
Prompt length, counted with the NVIDIA Nemotron 3 Nano 30B tokeniser over the training split: median **2,870 tokens**, mean **3,116**, longest **14,016**. The whole contract goes in every time, because any of the 17 items could be decided by any clause.
...
:::alert{header="Why completions start with <think>\n</think>" type="info"}
Nemotron 3 Nano is a reasoning model, and that creates a real problem out of the box. Its chat template automatically opens a `<think>` block at the start of every assistant turn, but nothing closes it. On a 17-item checklist, the model fills its entire generation budget with internal reasoning and never produces the JSON. Every metric returns 0.

The fix: every training completion starts with `<think>\n</think>\n`. The model learns to close the block the template opened, then produce the answer. At inference, it emits `</think>` and goes straight into the JSON. See `nemotron_support.py` for the measured numbers.
:::
...
:::alert{header="About `DataSet`" type="info"}
`sagemaker.ai_registry.dataset.DataSet` is a versioned dataset entry in the **SageMaker AI Registry**. It stores a pointer to an S3 object (bucket + key) and a name. No data is copied. `DataSet.create()` validates the file format against the declared technique, then registers the entry. `DataSet.get(name=...)` retrieves the latest version. Downstream resources (`SFTTrainer`, `CustomScorerEvaluator`) accept a `DataSet` object directly and record which version was used as lineage. [API documentation](https://docs.aws.amazon.com/sagemaker/latest/dg/serverless-customization-datasets.html)
:::
...
`CustomizationTechnique` is an enum from `sagemaker.ai_registry.dataset_utils` with values `SFT`, `DPO`, and `RLVR`. It is stored as a searchable keyword on the dataset entry (not a constraint), so you can filter datasets by technique in the AI Registry console. Evaluation datasets (like the test split here) are registered without a technique because they are not tied to a training method.
